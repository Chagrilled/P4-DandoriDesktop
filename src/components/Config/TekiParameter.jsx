...
export const TekiParameter = () => {
    const { config, configData, selectedRow, setConfigData } = useContext(ConfigContext);
    console.log("Selected config:", configData[config.name][selectedRow]);
    ...
    return <div className='bg-sky-1200 w-full h-full p-8 flex flex-wrap flex-col'>
        ...
        <div className='py-2' htmlFor={"FreezeDropType"} data-tooltip-id={"FreezeDropType"}>
            <Tooltip id={"FreezeDropType"} place={"top"}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                    Corresponds with values in DT_FreezeDropItem.json to determine how much nectar an enemy should drop
                </div>
            </Tooltip>
            <MarkerIcon type="gimmick" id="coldbox" size="xs" tooltip="" />
            <b>FreezeDropType</b>:&nbsp;
            <select
                className="bg-sky-1000 rounded-md px-3 py-0.5 text-[#e0e6ed] border border-[#3a4a5a] focus:ring-2 focus:ring-[#4da6ff] transition"
                value={data.FreezeDropType}
                onChange={e => onChange(e.target.value, "FreezeDropType")}
            >
                {Object.entries(FreezeDropTypes).map(([k, v]) =>
        ...
        <ConfigInput iconType="item" iconId="thunderfall" value="ThunderStopTime" type="number" tooltip="How long an enemy will be paused when struck with a Lightning Strike." />
