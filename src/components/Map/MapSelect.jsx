...
export const MapSelect = ({ }) => {
    ...
    const breakPointMaps = ['Area500', 'Cave035_F09'];
    ...
    const mapLinks = maps.map(mapId => {
        ...
        return <div key={mapId} style={breakPointMaps.includes(mapId) ? { 'borderBottom': '3px solid rgb(155, 175, 199)', 'marginBottom': '4px', 'paddingBottom': '4px', 'marginRight': '16px' } : undefined}>
            <button
                className={currentMap == mapId ? "font-bold" : ""}
                onClick={() => { onMapChange(mapId); return false; }}>
                {mapName}
                <i>
                    {` (${mapId})`}
                </i>
